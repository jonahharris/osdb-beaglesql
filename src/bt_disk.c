static const char  rcsfile []= "$Header: /home/magrasjc/test/src/RCS/bt_disk.c,v 0.87.2.1 2000/02/05 06:13:06 magrasjc Exp $";
/*
...
static int bt_blwrite(Btree *bt, int f, BufferList *bl)
{
    ...
    if (fseek(bt->fdata[f].file, bl->offset, SEEK_SET))
        ret = TREE_FAIL;
    if (!ret &&
        fwrite(bl->buffer,
        SIZES(f,block_size), 1, bt->fdata[f].file) != 1)
          ret = TREE_FAIL;
...
static int bt_blread(Btree *bt, int f, BufferList *bl)
{
    ...
    SHOW(printf("filename: %s, file: %Fp\n",
                        bt->fdata[f].filename, bt->fdata[f].file);)
...
static BufferList *bt_getbuf(Btree *bt, int f, DISKOFFSET dof)
{
    ...
    for (bl = bt->fdata[f].bufferlist; bl; ) {
        if (bl->state == sFree) {
            free_p = parent;
            free_b = bl;
        }
            ...
            if (parent) { /* move to top of list */
                parent->next = bl -> next;
                bl -> next = bt->fdata[f].bufferlist;
                bt->fdata[f].bufferlist = bl;
            }
            return bl;
        }
    ...
    if (free_b && parent) { /* Move new buffer to head of list */
        ...
        free_b -> next = bt->fdata[f].bufferlist;
        bt->fdata[f].bufferlist = free_b;
    }
...
/*
    if (f == DAT) 
      lprintf ("bt_getblock4read DATA");
    else
      lprintf ("bt_getblock4read INDEX");
*/
...
/*
    if (f == DAT) 
      lprintf ("bt_getblock4write DATA");
    else
      lprintf ("bt_getblock4write INDEX");
*/
...
/*
    if (f == DAT) 
      lprintf ("bt_getnew4write DATA");
    else
      lprintf ("bt_getnew2write INDEX");
*/

    /* first, are then any empty blocks already available? */
    if ((dof = bt->fdata[f].free_block) != 0) {
        SHOW(printf("bt_getnew4write(%Fp, %d, %ld)\n",bt,f,dof);)
        bl = bt_getbuf(bt, f, dof);
        if (!bl)
            return bl;
        bc = (BlockControl *) bl->buffer;
        bt->fdata[f].free_block = bc->next;
        bt->fdata[f].modified = 1;
    }
    else {
        /* extend the file */
        if (fseek(bt->fdata[f].file, 0L, SEEK_END))
            return NULL;
        dof = ftell(bt->fdata[f].file);

        /* just write something */
        if (fwrite(bt->fdata[f].bufferlist,
                         SIZES(f,block_size),
                        1,
                        bt->fdata[f].file) != 1) {
            bt -> error_code = 10;
...
int bt_flush(Btree *bt) {
    ...
    ret = TREE_OK;
    for (i=0; i<2; i++) {
        if (bt->fdata[i].file && bt->fdata[i].bufferlist) {
            char *p;
            DISKOFFSET *d;
            BufferList *bl;

            /* root block data changed? */
            if (bt->fdata[i].modified) {
                ...
                p += sizeof(Sizes);
                d = (DISKOFFSET *) p;
                *d = bt->fdata[i].root_block;
                d++;
                *d = bt->fdata[i].free_block;
                bt->fdata[i].modified = 0;
            }

            for (bl = bt->fdata[i].bufferlist; bl; bl = bl->next) {
                if (bl->state == sDirty)
                    ret = bt_blwrite(bt, i, bl);
            }

            fflush(bt->fdata[i].file);
        }
    }
...
int bt_bufinit(Btree *bt, int f)
{
    ...
    if (bt->fdata[f].bufferlist == NULL) {
        ...
        bt->fdata[f].bufferlist = dummy.next;
...
int  bt_releaseblock(Btree *bt, int f, DISKOFFSET dof)
{
    ...
    bc -> next = bt->fdata[f].free_block;
    bc -> blockstate = sAvail;
    bt->fdata[f].free_block = dof;
    bt->fdata[f].modified = 1;
