static const char  rcsfile []= "$Header: /home/magrasjc/test/src/RCS/bt_data.c,v 0.87.2.1 2000/02/05 06:13:04 magrasjc Exp $";
/*
...
#include "bt_hdr.h"
#include "store.h"   /* prototypes for offset storage routines */
...
int deleting=NO;  /* flag to tell the bt_walk_down routine whether or not a 
                     delete operation is to be preformed */
int FoundKey=NO;  /* flag added to assist in the delete operation */
...
int FindFunc(Btree *bt, void *rec) {
    ...
    if (compare > 0)
        ...
        if (bt->FoundRec) {
            ...
            store_offset (rec);  /* store offset of this record */
        }
        else {
            /*
                 ...
                 int mode)          /* SELECTIVE or EXHAUSTIVE */
{
    ...
    if (f == NDX) {
        ...
        while (offset < bc->bfree) {
            ...
            memcpy (&datablock, s+offset+keysize, sizeof(DISKOFFSET));
        ...
        while (offset < bc->bfree) {
            ...
            if (retval || (!retval && deleting && FoundKey))
                break;
...
int bt_walk(Btree *bt, DoFunc df) {
    ...
    return bt_walk_down(
        bt, NDX, bt->fdata[NDX].root_block, df, 0, EXHAUSTIVE);
}
...
int bt_find(Btree *bt, void *rec, void *key) {
    ...
    bt->FoundRec = rec;
    retval = bt_walk_down(
        bt, NDX, bt->fdata[NDX].root_block, FindFunc, 0, SELECTIVE);
                         ...
                         DISKOFFSET newblock)
{
    ...
    if (bc->bfree + delta > SIZES(NDX,split)) {
        ...
        while (offset < bc->bfree/2) {
            ...
            memcpy (&tmp, pblock+offset+keysize, sizeof(DISKOFFSET));
            /*if (*((DISKOFFSET *)(pblock + offset + keysize)) == oldblock)*/
            if (tmp == oldblock)
                oldblockseen = 1;
            offset += keysize + sizeof(DISKOFFSET);
        }
        ...
        while(offset < bc1 -> bfree) {
            ...
            memcpy (&goal, pblock1+offset+keysize, sizeof(DISKOFFSET));
    ...
    if (replacekey) { /* change keys but not DISKOFFSETs */
        ...
        if (newkey) { /* if NULL, we are actually deleting */
            ...
            memcpy (pblock+offset+newsize, &oldblock, sizeof(DISKOFFSET));
        ...
        memcpy (pblock+offset+newsize, &newblock, sizeof(DISKOFFSET));
  ...
  top:
    ...
    if (bc->bfree + datasize > SIZES(DAT,split)) {
        if (bt_split(bt))
            ...
            retval = bt_walk_down(
                bt, NDX, bt->fdata[NDX].root_block,
...
DISKOFFSET bt_find_sister(Btree *bt,
    ...
    while (offset < bc -> bfree) {
        keysize = GETKEYSIZE(pblock + offset);
        memcpy (&dof, pblock+offset+keysize, sizeof(DISKOFFSET));
    ...
    if (direction == LEFT) {
        if (lastoffset) { /* it is in this block */
            ...
            memcpy (&dof, pblock+lastoffset+keysize, sizeof(DISKOFFSET));
            /* dof = *((DISKOFFSET *)(pblock + lastoffset + keysize)); */
    ...
    if (bcleft->bfree + bcright->bfree - sizeof(BlockControl) <
        ...
        if (f == NDX) {
            ...
            while(offset < moving) {
                ...
                memcpy (&goal, pright+offset+keysize, sizeof(DISKOFFSET)); 
                /*goal = *((DISKOFFSET *)(pright + offset + keysize));*/
...
int bt_delete(Btree *bt, void *key)
{
    ...
    deleting = YES;  /* yes I want to delete a key */
    bt->SearchKey = key;
    bt->FoundRec = NULL;
    retval = bt_walk_down(
        bt, NDX, bt->fdata[NDX].root_block, FindFunc, 0, SELECTIVE);
    deleting = NO;   /* reset the delete flag */
    if (!retval && bt->FoundRec)
