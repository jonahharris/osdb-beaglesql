...
struct clist {
   ...
   void *value;           /* used by expresstion parser for variable values */
   int type;              /* data type */
   struct clist *next;
};

extern struct clist *parse_tokens (char *s);
extern void free_tokens (struct clist *tokens);
extern int BOOLEAN_get_exp (struct clist *tokens);
extern int ARITHMETIC_get_exp (struct clist *tokens);
